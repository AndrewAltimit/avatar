...
| `avatar fbx reslot <path> --mesh M --to-slot N …` | Move a region of a mesh's polygons onto another material slot (select by `--from-slot`, `--near-bone/--radius`, `--min-z/--max-z`, `--exclude-bone`, `--brighter-than TEX:LUM`, `--any-corner`); writes the FBX with `-o` (dry run without). The "that strand glows white" fix. |
