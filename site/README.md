...
| `js/analyzer.js` | The inspector page: drag-and-drop FBX → the wasm report, rendered as tabs; owns the `TextureLibrary` (image files / folders dropped alongside the `.fbx`, matched to materials by the avatar `.prefab`'s per-renderer-slot material guid, then a Unity `.mat` named after the material → `_MainTex` guid → `.meta` guid, then by basename, then by stem) and hands it to the viewer. |
