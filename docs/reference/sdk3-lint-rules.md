...
| `VRC062` | warn | A custom-layer animation drives a blendshape the descriptor **registers as an eyelid** (`eyelidType: Blendshapes` + `eyelidsBlendshapes` index) — VRChat's eyelid driver writes registered shapes *after* the animators every frame, so the animation is silently overridden in-game (the classic "my blink expression does nothing"). Fires regardless of `enableEyeLook`. Resolves eyelid mesh → `m_Mesh` guid → source FBX → morph channels in **import order** (how Unity numbers blendshapes) → names at the registered indices, then scans the custom layers' clips (state motions + blend-tree children) for non-zero `blendShape.<name>` curves; unresolvable steps return quietly. Note the SDK enum: `eyelidType` 0 None, 1 Bones, **2 Blendshapes** | [eye look](https://creators.vrchat.com/avatars/avatar-descriptor/#eye-look) |
