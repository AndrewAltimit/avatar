# PhysBone tuning — `avatar physbone list|set|split|stretch`
...
`parameter` — each with its **curve** if one is set.
...
**Preview before Unity:** `avatar render --avatar model.fbx --stretch 'Skirt_0_*:1.5' -o
out.png` applies the same edit to the FBX for the picture — `HINGE:FACTOR`, `*` wildcards, every
bone below the matching hinges is scaled ([render.md](render.md)); the two commands describe the
same edit from their own side (prefab: below the PhysBone root; FBX: below the named hinges).
...
pigtail with curves, stretches the skirt, and pins the result (`Sdk2Project.physbones.tuned.json`)
